...
→ **決定（2026-09-21）**: 消耗型の投げ銭 3 段（`cc.nyoyapoya.denoncontroller.tip.small` /
`.tip.medium` / `.tip.large`）。価格は App Store Connect で決める。

- **設置場所**: 設定の「ヘルプとフィードバック」に「開発を応援する」を常設。加えて、接続成功 10 回
  以上かつ初回成功から 14 日以上の人に一度だけアラートで案内する（評価依頼と同じ日には出さない）。
- **サポーター特典**: 応援した人の要望は優先的に検討する。消耗型は StoreKit に履歴が残らないため、
  支援した事実を iCloud キーチェーンに記録し（`SupporterRecord`）、「ご意見・ご要望を送る」から送ると
  Worker が Issue に `supporter` ラベルを付ける。Worker は公開エンドポイントなので、ラベルは
  購入の証明ではなくトリアージの目安として扱う。
- **文言**: 「優先的に検討します」とし、「必ず対応」とは書かない。応援しても機能は変わらないことを
  画面と審査メモに明記する。
