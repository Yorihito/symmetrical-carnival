...
一度だけの案内を出す条件:
- 接続中の機種が、対応機種の一覧（4.4 の JSON）で **開発者確認済みでも報告済みでもない**、または報告が 3 件未満
- その機種への接続に 3 回以上成功している（ある程度使ってから聞く）
- この端末からその機種の動作報告をまだ送っていない（送った機種を UserDefaults に記録）
- 今日、レビューのお願いや応援のお願いを出していない（`ReviewRequestManager` / `SupportRequestManager` と同じ「1 日 1 つまで」の決まりに従う）
...
- 一度だけの案内を、報告が何件そろった機種で止めるか（案: 「問題なく使える」が 3 件）
