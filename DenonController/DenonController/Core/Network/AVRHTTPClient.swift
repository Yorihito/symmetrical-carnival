...
actor AVRHTTPClient {
    ...
    private func bsdGET(path: String, host: String, port: Int) async throws -> (Data, Int) {
        do {
            ...
            throw err    private static func bsdGETBlocking(host: String, port: Int, path: String) throws -> (Data, Int) {
        let start = Date()
        print("[DenonLog] > bsdGET start: \(host):\(port)\(path)")
        ...
        guard gaiRet == 0, let first = resPtr else {
            print("[DenonLog] ! getaddrinfo failed: \(gaiRet)")
            throw AVRError.connectionFailed("アドレス解決失敗 (\(host)): \(gaiRet)")
        }
        defer { freeaddrinfo(resPtr) }
        print("[DenonLog] > getaddrinfo success (\(String(format: "%.3fs", Date().timeIntervalSince(start))))")
        ...
        guard connectRet == 0 else {
            ...
            print("[DenonLog] ! connect failed: \(err) (\(String(format: "%.3fs", Date().timeIntervalSince(connectStart))))")
            throw AVRError.connectionFailed("\(host):\(port) — \(err)")
        }
        print("[DenonLog] > connect success (\(String(format: "%.3fs", Date().timeIntervalSince(connectStart))))")
        ...
        print("[DenonLog] > recv success (\(responseData.count) bytes, total: \(String(format: "%.3fs", Date().timeIntervalSince(start))))")
        let status = parseHTTPStatus(data: responseData)
        return (responseData, status)
    }ocket APIs — curl と同じレイヤーで接続する
...
─────────────────────────────────────
    ...
    private static func bsdPOSTBlocking(host: String, port: Int, path: String, body: String) throws -> (Data, Int) {
        let start = Date()
        print("[DenonLog] > bsdPOST start: \(host):\(port)\(path)")

        var hints = addrinfo(ai_flags: AI_DEFAULT, ai_family: AF_INET, ai_socktype: SOCK_STREAM, ai_protocol: 0, ai_addrlen: 0, ai_canonname: nil, ai_addr: nil, ai_next: nil)
        var resPtr: UnsafeMutablePointer<addrinfo>?
        let gaiRet = getaddrinfo(host, String(port), &hints, &resPtr)
        guard gaiRet == 0, let first = resPtr else {
            print("[DenonLog] ! POST getaddrinfo failed: \(gaiRet)")
            throw AVRError.connectionFailed("アドレス解決失敗 (\(host)): \(gaiRet)")
        }
        defer { freeaddrinfo(resPtr) }
        ...
        guard connectRet == 0 else {
            let err = String(cString: strerror(errno))
            print("[DenonLog] ! POST connect failed: \(err)")
            throw AVRError.connectionFailed("\(host):\(port) — \(err)")
        }
        print("[DenonLog] > POST connect success (\(String(format: "%.3fs", Date().timeIntervalSince(connectStart))))")
        ...
        let status = parseHTTPStatus(data: responseData)
        print("[DenonLog] > POST recv success (HTTP \(status), \(responseData.count) bytes, total: \(String(format: "%.3fs", Date().timeIntervalSince(start))))")
        return (responseData, status)
    }ocket APIs — curl と同じレイヤーで接続する
