...
actor AVRHTTPClient {
    ...
    private static func bsdGETBlocking(host: String, port: Int, path: String) throws -> (Data, Int) {
        let start = Date()
        print("[DenonLog] > bsdGET \(host)\(path)")
        ...
        guard gaiRet == 0, let first = resPtr else {
            print("[DenonLog] ! getaddrinfo error: \(gaiRet)")
            throw AVRError.connectionFailed("アドレス解決失敗 (\(host)): \(gaiRet)")
        }
        defer { freeaddrinfo(resPtr) }
        print("[DenonLog] > getaddrinfo OK (\(String(format: "%.3f", Date().timeIntervalSince(start)))s)")
        ...
        */
        ...
        guard connectRet == 0 else {
            ...
            print("[DenonLog] ! connect failed: \(err) (\(String(format: "%.3f", Date().timeIntervalSince(connectStart)))s)")
            throw AVRError.connectionFailed("\(host):\(port) — \(err)")
        }
        print("[DenonLog] > connect OK (\(String(format: "%.3f", Date().timeIntervalSince(connectStart)))s)")
        ...
        print("[DenonLog] > recv OK (\(responseData.count) bytes, total \(String(format: "%.3f", Date().timeIntervalSince(start)))s)")
